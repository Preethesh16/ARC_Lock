...
void markAttendance(String cardID) {
  ...
  struct tm *ptm = gmtime ((time_t *)&epochTime);
	...
	String month_year = String(ptm->tm_mon+1)+"-"+String(ptm->tm_year+1900);
	...
	if(currentDate != access_time){
		String name = Firebase.getString(fbdo, "access/" + cardID + "/name") ? fbdo.to<const char *>():"blank";

		String path = "/attendance/"+month_year+"/" + name +"/count";
